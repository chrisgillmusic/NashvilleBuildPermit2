...
export function PermitFeedCard({ project, trade, expanded, onToggle }: Props) {
  ...
  return (
    <>
      ...
      {expanded ? (
        <div className="pointer-events-auto fixed inset-x-0 top-0 z-40 h-[calc(100vh-6.5rem)] translate-y-0 opacity-100 transition-all duration-300">
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={onToggle} />
          <div className="absolute inset-x-3 bottom-0 top-3 overflow-hidden rounded-[30px] border border-white/10 bg-[#111113] shadow-[0_30px_90px_rgba(0,0,0,0.55)]">
